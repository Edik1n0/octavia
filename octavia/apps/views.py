...
from .navegacion.views import (
    contactoView,
    productsDetailView,
    recoverypasswordoctavia,
    registeroctavia,
)
from .utils import send_mail_octavia
...
class ClienteViewMixin(FormMixin, registeroctavia):
    ...
    def form_valid(self, form):
        ...
        send_mail_octavia(ctx, self.request)
        form.save()
        self.response = True
        messages.add_message(
            self.request,
    ...
    def get_success_url(self):
        msg = "ok"
        return "{}?msg={}".format(reverse("registro_octavia"), msg)


def activate_octavia(request, unicknameb64):
    try:
        unickname = force_str(urlsafe_base64_decode(unicknameb64))
        cliente = Cliente.objects.get(nick_name=unickname)
    except (TypeError, ValueError, OverflowError, Cliente.DoesNotExist):
        cliente = None

    if cliente:
        cliente.is_octavia = True
        cliente.fecha_activacion = timezone.now()
        cliente.save()
        titulo = str("FORMULARIO DE PRE CONFIRMACIÓN CLIENTE OCTAVIA")
...
class RecoveryPasswordViewMixin(FormMixin, recoverypasswordoctavia):
